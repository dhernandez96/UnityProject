...
public class PlayerController2 : MonoBehaviour
{
    ...
    private int totalWindFlowers = 1;
    private int jumpsRemaining; 
    private SpriteRenderer spriteRenderer;
    private bool isWindMario = false;
    private int livesUntilBoost = 3;
    ...
    void Update()
    {
        ...
        // When the player presses the spacebar & is Wind Mario, ShootGust()
    ...
    void OnCollisionEnter2D(Collision2D col)
    {
    	...
    	if (col.gameObject.name == "Ground" ||
            ...
            col.gameObject.name == "QuestionBlockTop")
    	{
    		jumpsRemaining = totalJumps;
   		}
        
        //instantiate wind flower when hitting bottom of question block
        if(col.gameObject.name == "QuestionBlockBottom")
        {
            if(totalWindFlowers > 0)
            {
                Instantiate(windFlower, new Vector3(-7, 2, 0), new Quaternion(0, 0, 0, 0));
                totalWindFlowers--;
            }
        }
    }
    ...
    void OnCollisionExit2D(Collision2D col)
    {
        ...
        if (col.gameObject.name == "HammerBro1" || col.gameObject.name == "HammerBro2")
        {
            ...
            if (health == 0)
            {
                ...
                health = 2;
                livesUntilBoost--;
                if(livesUntilBoost <= 0)
                {
                    Instantiate(questionBlock, new Vector3(-7, 1, 0), new Quaternion(0, 0, 0, 0));
                    totalWindFlowers = 1;
                }
            }
        }
    ...
    {
        if (col.gameObject.name == "WindFlower(Clone)")
        {
